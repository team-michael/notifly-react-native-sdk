...
# Notifly React Native LLMS generator (outputs llms.txt)
# Usage:
...
# Priority:
#  1) Known roots if present: src, android, ios, example (and samples if exists)
...
if [[ -z "${INCLUDE_DIRS:-}" ]]; then
  ...
  [[ -f ".npmrc" ]] && dirs+=(".npmrc")
  [[ -f ".nvmrc" ]] && dirs+=(".nvmrc")
  [[ -f ".eslintIgnore" ]] && dirs+=(".eslintIgnore")
  [[ -f ".eslintrc" ]] && dirs+=(".eslintrc")
  # cocoapods spec
  for podspec in *.podspec; do
    [[ -f "$podspec" ]] && dirs+=("$podspec")
  done
  if [[ ${#dirs[@]} -gt 0 ]]; then
    INCLUDE_DIRS="${dirs[*]}"
  else
...
path_is_target() {
  ...
  case "$base" in
    README.md|CHANGELOG.md|LICENSE|package.json|tsconfig.json|tsconfig.build.json|babel.config.js|lefthook.yml|jest.config.js|.npmrc|.nvmrc|.eslintIgnore|.eslintrc|*.podspec) return 0 ;;
  esac
  # Check extension
  local ext="${rel##*.}"
  local match_ext=0
